...
abstract class DatabaseDriver implements DatabaseInterface, Log\LoggerAwareInterface
{
	...
	public static function getConnectors()
	{
		if (!isset(self::$connectors))
		{
			...
			foreach ($iterator as $file)
			{
				...
				$class = '\\Smoke\\Database\\' . ucfirst(strtolower($baseName)) . '\\' . ucfirst(strtolower($baseName)) . 'Driver';
	...
	public static function getInstance($options = array())
	{
		...
		if (empty(self::$instances[$signature]))
		{
			...
			$class = '\\Smoke\\Database\\' . ucfirst(strtolower($options['driver'])) . '\\' . ucfirst(strtolower($options['driver'])) . 'Driver';
	...
	{
		// Derive the class name from the driver.
		$class = '\\Smoke\\Database\\' . ucfirst($this->name) . '\\' . ucfirst($this->name) . 'Exporter';
	...
	{
		// Derive the class name from the driver.
		$class = '\\Smoke\\Database\\' . ucfirst($this->name) . '\\' . ucfirst($this->name) . 'Importer';
	...
	public function getQuery($new = false)
	{
		...
		{
			// Derive the class name from the driver.
			$class = '\\Smoke\\Database\\' . ucfirst($this->name) . '\\' . ucfirst($this->name) . 'Query';
	...
	{
		// Derive the class name from the driver.
		$iteratorClass = '\\Smoke\\Database\\' . ucfirst($this->name) . '\\' . ucfirst($this->name) . 'Iterator';
